# Parses the DTD file and creates a timetable for trains
import os
import csv
...
# 1. Run the R script (DTD_API_call.R)
r_script_path = os.path.join(script_dir, "DTD_API_call.r")
...
# 2. Unzip the timetable_data.zip file into a folder with the same name
zip_file_path = os.path.join(script_dir, "timetable_data.zip")  # Provide the correct path to your zip file
zip_base_name = os.path.splitext(os.path.basename(zip_file_path))[0]  # Get the base name of the zip file
extracted_folder_path = os.path.join(os.getcwd(), zip_base_name)  # Create a folder with the same name in the current directory

with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
    zip_ref.extractall(extracted_folder_path)

# Find the file in the extracted folder with a .mca extension
mca_file_path = None
for root, dirs, files in os.walk(extracted_folder_path):
    for file in files:
        if file.endswith(".MCA"):
            mca_file_path = os.path.join(root, file)
            print(mca_file_path)
            break  # Stop searching once a .mca file is found

if mca_file_path:
    print(f"Found .mca file: {mca_file_path}")
else:
    print("No .mca file found in the extracted folder")
...
output_directory = extracted_folder_path
extension = ".csv"
...
timetable_csv_path = f"{output_directory}/KingsCross_Edinburgh_Future_Timetable.csv"
...
# check for timetable_data.zip in folder

  
from_date = datetime.strptime('2023-12-02', '%Y-%m-%d')
to_date = datetime.strptime('2024-03-02', '%Y-%m-%d')
